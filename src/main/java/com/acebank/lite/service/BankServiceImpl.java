...
public class BankServiceImpl implements BankService {
    ...
    public String withdraw(int accountNo, BigDecimal amount) {
        try {
            ...
            // Balance Check
            BigDecimal currentBalance = userDao.getBalance(accountNo);
        ...
        try {
            ...
            if (detailsOpt.isPresent()) {
                ...
                String msg = "Hi " + details.firstName() + " " + details.lastName() + ",\n\n"
                        ...
                        + "Please use this OTP to change your password. Do not share this OTP with anyone.\n\n"
                        + "Best regards,\nAceBank Team";
            ...
            log.severe("Failed to send OTP for: " + email + " Error: " + e.getMessage());
        }
        return null;
    }
    ...
    public boolean updatePasswordByEmail(String email, String newPassword) {
        try {
            Optional<AccountRecoveryDTO> detailsOpt = userDao.getRecoveryDetails(email);
            if (detailsOpt.isPresent()) {
                int accountNo = detailsOpt.get().accountNo();
                String newSecureHash = PasswordUtil.hashPassword(newPassword);
                return userDao.changePassword(accountNo, null, newSecureHash);
            }
        } catch (SQLException e) {
            log.severe("Database error updating password for email: " + email);
        }
        return false;
    }
